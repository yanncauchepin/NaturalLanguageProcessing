...
"""Word2vec model is configurable in the gensim library with parameters :
...
- size parameter corresponding to the vector size. Default value is 100.
- sg parameter corresponding to a boolean to select the method applied among
Skip-gram (1) or CBOW (0).
- negative parameter which when greater than 0, indicates that negative sampling
should be used and the integer value signifies the number of negative samples
to use.
- worker parameter which defines the number of threads to use for training.
The save() method could be applied to store as pickle file """
